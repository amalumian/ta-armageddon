...
export default function AsteroidList({
  ...
  return asteroids.map((obj) =>
    obj.data.map((asteroid) => {
      const date = formatDate(obj.date);

      const distance = {
        km: formatDistanceInKm(
          asteroid.close_approach_data[0].miss_distance.kilometers,
        ),
        lunar: formatDistanceInLunar(
      ...
      const diameterMin: number =
        asteroid.estimated_diameter.meters.estimated_diameter_min;
      const diameterMax: number =
        asteroid.estimated_diameter.meters.estimated_diameter_max;
      const diameterAverage = formatDiameterInM(
